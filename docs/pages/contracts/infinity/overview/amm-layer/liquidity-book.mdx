...
<Callout type="info">
If a trade happen within a bin, it will incur 0 slippage. However if the trade consume all the liquidity within the bin and cross over to the next bin, there will be slippage.
</Callout>
...
```solidity
// ref: BinPoolManager.sol
function getBin(PoolId id, uint24 binId) external view returns (uint128 binReserveX, uint128 binReserveY, uint256 binLiquidity, uint256 totalShares);
```
...
Fungible liquidity also provides flexibility in downstream defi application. When LPs add liquidity via `BinPositionManager.sol`, they get ERC1155 token.
...
See [BinPositionManager](https://github.com/pancakeswap/infinity-periphery/blob/main/src/pool-bin/BinPositionManager.sol) on how a token is minted to the liquidity provider.