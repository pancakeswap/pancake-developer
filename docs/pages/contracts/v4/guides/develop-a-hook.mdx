...
1. We need to first store the default swap in the hook. We can do during pool initialization, passing the swap fee in the hookData which will flow to the hook. The hook can abi.decode the hookData and store the swap fee.

![Store default swap fee in hook](/v4/guides-develop-hook-2.png)

2. With the hook fee stored, at `beforeSwap()`, hook check if the swapper is veCake holder and return the updated swap fee accordingly through `lpFeeOverride` value.
...
Create a file called at `src/pool-cl/VeCakeDiscountHook.sol` and implement the following. The hook contract extends `CLBaseHook`.
...
`beforeSwap` will be called before a swap happens, and the third return value `uint24` is where we can override the swap fee.
...
Create a file called at `test/pool-cl/VeCakeDiscountHook.t.sol` and copy content from below.
