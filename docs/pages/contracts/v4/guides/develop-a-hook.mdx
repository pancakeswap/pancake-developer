...
Refer to https://github.com/pancakeswap/pancake-v4-hooks-template for the hook template. Simply click `Use this template` to create a new repository based on the template.

Once the new repository is cloned to local setup, run the following commands: 
```
> forge install // install dependencies
> forge test // run the existing CounterTest in the repository
```

Within both `src` and `test` there are 2 folders: `pool-cl` and `pool-bin`. If you are developing for concentrated liquidity pool, focus on pool-cl folder, otherwise bin pool for pool bin.

### BaseHook
BaseHook is a base contract where you will inherit for your hooks. It provides 

1. helper `_hooksRegistrationBitmapFrom` to set up the callback required
...
Idea: [veCake](https://docs.pancakeswap.finance/products/vecake/what-is-vecake) holder will get a 50% swap fee discount when swapping through pool with this hook.
...
### Step 2: Introduce the implementation idea

The flow will be as follows:

1. Before a swap, pool manager call `hook.beforeSwap()` 
2. At `beforeSwap()`, hook call `CLPoolManager.updateDynamicSwapFee()`.
3. At `CLPoolManager.updateDynamicSwapFee()`, pool manager will call `hook.getFee()` to fetch the swap fee. This is where our hook will check and return either default swap fee or 50% discount. Pool manager will save the returned value and set that as swap fee for the pool.
...
2. call `poolManager.updateDynamicSwapFee(key)`
3. implement both `beforeSwap` and `getFee` method
