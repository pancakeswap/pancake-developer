...
When liquidity breing added to the current active binId is not the current bin tokenY and tokenX ratio, a "swap" will occur when liquidity is being added. This "swap" will incur composition fee. The fee is the same as typical swap (lp + protocol) fee.

The purpose of composition fee is to prevent user leveraging add/remove liquidity to perform a "free swap".
